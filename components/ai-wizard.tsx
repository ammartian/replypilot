...
const STEPS = ['Business', 'Language', 'Tone & Style', 'Qualification', 'Handoff', 'Instructions']
...
function OnOff({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <div className="flex gap-1">
      <ToggleButton active={value} onClick={() => onChange(true)}>On</ToggleButton>
      <ToggleButton active={!value} onClick={() => onChange(false)}>Off</ToggleButton>
    </div>
  )
}
...
export function AiWizard({
  ...
  function toggleField(field: QualificationField) {
    if (selectedFieldKeys.has(field)) {
      update('qualificationFields', answers.qualificationFields.filter((f) => f.field !== field))
    } else {
  ...
  function moveField(field: QualificationField, dir: 'up' | 'down') {
    ...
    update('qualificationFields', sorted.map((f, i) => ({ ...f, order: i })))
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaved(true)
      setTimeout(() => onSaved(preview), 800)
    } catch (err) {
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Select value={answers.industry} onValueChange={(v) => update('industry', v as Industry)}>
            ...
            <div className="space-y-2">
              <Label htmlFor="biz-desc">
                ...
                <span className="text-neutral-400 font-normal">({answers.businessDescription.length}/200)</span>
              </Label>
              <textarea
                id="biz-desc"
                rows={3}
          ...
          <CardContent className="space-y-5">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => update('primaryLanguage', v as WizardAnswers['primaryLanguage'])}
              >
                <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            {answers.primaryLanguage === 'ms' && (
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-neutral-400">Enforce standard Malaysian Malay, not Indonesian variants</p>
                </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-neutral-400">Allow mixing languages to match the buyer</p>
              </div>
              <OnOff value={answers.allowCodeSwitching} onChange={(v) => update('allowCodeSwitching', v)} />
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {([
                  ...
                  <ToggleButton key={opt.value} active={answers.tone === opt.value} onClick={() => update('tone', opt.value)}>
                    {opt.label}
                  </ToggleButton>
                ))}
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <ToggleButton active={answers.replyLength === 'short'} onClick={() => update('replyLength', 'short')}>
                  Short (1-2 sentences)
                </ToggleButton>
                <ToggleButton active={answers.replyLength === 'medium'} onClick={() => update('replyLength', 'medium')}>
            ...
            <div className="flex items-center justify-between">
              ...
              <OnOff value={answers.emojiAllowed} onChange={(v) => update('emojiAllowed', v)} />
            </div>
            <div className="flex items-center justify-between">
              <div>
              ...
              <OnOff value={answers.fillerPhrasesAllowed} onChange={(v) => update('fillerPhrasesAllowed', v)} />
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {step === 3 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Choose what info to collect and in what order.</CardDescription>
          </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {ALL_QUALIFICATION_FIELDS.map(({ field, label }) => (
                  <ToggleButton key={field} active={selectedFieldKeys.has(field)} onClick={() => toggleField(field)}>
                    {label}
                  </ToggleButton>
                ))}
              </div>
              ...
              <>
                ...
                <div className="space-y-1">
                  ...
                  {selectedFields.map((f, i) => {
                    ...
                    return (
                      <div key={f.field} className="flex items-center gap-2 py-1.5 px-2 rounded-md bg-neutral-50 dark:bg-neutral-800/50">
                        <span className="text-xs text-neutral-400 w-4 text-right">{i + 1}.</span>
                        <span className="text-sm flex-1">{label}</span>
                        <ToggleButton active={f.required} onClick={() => toggleRequired(f.field)} className="text-xs py-0.5 px-2">
                          {f.required ? 'Required' : 'Optional'}
                        </ToggleButton>
                        <div className="flex gap-0.5">
                          <button type="button" onClick={() => moveField(f.field, 'up')} disabled={i === 0} className="p-1 rounded text-neutral-400 hover:text-neutral-600 disabled:opacity-20">
                            <ArrowUp className="w-3 h-3" />
                          </button>
                          <button type="button" onClick={() => moveField(f.field, 'down')} disabled={i === selectedFields.length - 1} className="p-1 rounded text-neutral-400 hover:text-neutral-600 disabled:opacity-20">
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <ToggleButton active={answers.handoffTrigger === 'hot_warm'} onClick={() => update('handoffTrigger', 'hot_warm')}>Hot or Warm</ToggleButton>
                <ToggleButton active={answers.handoffTrigger === 'hot'} onClick={() => update('handoffTrigger', 'hot')}>Hot only</ToggleButton>
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {([
                  ...
                  <ToggleButton key={opt.value} active={answers.handoffMessagePreset === opt.value} onClick={() => update('handoffMessagePreset', opt.value)} className="w-full text-left justify-start">
                    {opt.label}
                  </ToggleButton>
                ))}
              </div>
      ...
      {step === 5 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Claude synthesizes your choices into instructions. Edit freely before saving.
            </CardDescription>
          </CardHeader>
